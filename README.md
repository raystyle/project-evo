...
> 一句话定位:项目治理插件市场仓。市场名 `project-evo`,源 `raystyle/project-evo`,四插件十 skill:`evo-adr`(文档治理:doc-gov、code-kit、gh-issue、build-release)、`evo-codesec`(代码安全:secret-scan、security-audit)、`evo-research`(研究检索:research、report)、`evo-herdr`(多仓协作:herdr-flywheel、herdr-review)。客户端显示 `<插件>:<skill>`。
...
- 钉版:Claude Code `raystyle/project-evo@v0.8.0`(或 URL 尾 `#v0.8.0`);Codex `--ref v0.8.0`(下一封版 v0.9.0 后同理)
...
| evo-adr | `build-release` | 编译打包发布流水线指导:六型配方(go/rust/npm/py/native/manifest)、公共契约、可拷改模板(workflow 骨架、rclone 双段、自升级核对清单) | 搭或改 CI/CD 流水线、发 Release、推镜像时 |
