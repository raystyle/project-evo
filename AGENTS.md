...
> 定位:项目治理插件市场仓(四插件八 skill 同属项目治理面,非业务工具面):evo-adr(文档治理:doc-gov 知识面、code-kit 工具面)、evo-codesec(代码安全:secret-scan、security-audit,后者译自 Cloudflare security-audit-skill,MIT,skill 目录内附 LICENSE)、evo-research(研究检索:research、report)、evo-herdr(多仓协作:herdr-flywheel 飞轮协议、herdr-review 评审闸门),市场名 project-evo,客户端显示 `<插件>:<skill>`。skill 硬性规范以 [agentskills 官方 spec](https://agentskills.io/specification) 为硬标准,细则见 `docs/guides/skill-spec.md`。
