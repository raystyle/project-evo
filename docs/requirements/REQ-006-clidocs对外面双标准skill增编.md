...
- [x] 乙面 agent-face 三件:--llms 手册面(裸 markdown 至多 120 行加 --llms --json 机器形,活命令树渲染禁手维护,全派生或 curated 加守卫两形);JSON 协议(--format 族加 --json 简写互斥,信封 ok/data|error/meta/hints,jsonl 无信封,错误 stderr 单行 JSON,退出码 0/1/2,字段序稳定,filter/fields 裁剪);自省(活命令树唯一真源,三面同源,漂移守卫锁全旗标覆盖)
- [x] 模板四节可拷改(README 骨架、手册骨架、信封 schema、自省核对清单),零私有名
