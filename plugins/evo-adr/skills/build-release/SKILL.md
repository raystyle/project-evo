...
> 标准通用件:任何仓按清单形态对号入座拿配方,正文与模板只含参数化占位符,不含任何具体环境私有名。**边界**:本 skill 只辖仓侧 GitHub CI/CD 自建流(编译、测试、打包、发布、自播);CI 不可产的大件分发不属本面。封版操作面(封版件、tag 触发、发布验收)在同插件 doc-gov 的 flow-release,互引不重复。
...
| 有 `Cargo.toml` | rust 形 | target matrix 五目标;自升级适用判据必答 |
...
1. 仓侧全链自包:编译、测试、打包、发布、播种五环全在本仓流水线;发布动作恒由流水线完成,人工手挂属过渡特例,禁为新形态
2. 镜像恒 rclone(env 四键 Secrets 形),双段制版本段加 stable 滚动段
...
| 可拷改模板(workflow 骨架、各型片段、rclone 镜像步、自升级核对清单) | `references/templates.md` |
