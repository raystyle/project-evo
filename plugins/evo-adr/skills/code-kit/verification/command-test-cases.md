...
> evo-adr 是文档治理插件(工具 skill 为 code-kit,体系知识在 doc-gov)；机检形态为 skill 内 `scripts/check.py`（PEP 723 零依赖，`uv run check.py <项目>` 直跑，退出码 0/1/2），本文件是其等价 PowerShell 用例集，验证一个项目是否遵守 project-evo 骨架。
...
# 注记: 中转态是否带验证路径、收尾是否处置悬空条目,属语义判断,机检不做,收尾手检(六态红线见 doc-gov 的 base-writing-standards)
...
$j = uv run skills\code-kit\scripts\check.py $ProjectRoot --json | ConvertFrom-Json
