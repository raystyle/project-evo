...
> Rust 栈的文档即代码落地。项目从零起步照此配；投影与状态机总纲见 doc-gov 的 base-projection 与 base-adr。
...
- 平台门控项（`cfg(windows)` 等）的契约注释在异平台本地不可见,`missing_docs` 覆盖结论须过交叉岗或 CI 平台矩阵验,单平台本机绿不作数 [实证: 首批强制仓 Linux 本地绿、交叉岗抓出 cfg-windows 漏注]
