...
> 五步执行指南 + 裁剪原则 + 验收清单 + 存量迁移。在目标项目执行 code-kit 骨架安装时照此走。
...
```powershell
uv run plugins/evo-adr/skills/code-kit/scripts/init.py <目标项目> --name <项目名>
```
...
3. 按项目类型追加：公开库配投影工具链（Rust aidoc / Python MkDocStrings / TS API Extractor,投影总纲见 doc-gov 的 base-projection,本目录 tool-* 分栈）；有脚本积累建 `.tools\` + README；README 缺则一并建
