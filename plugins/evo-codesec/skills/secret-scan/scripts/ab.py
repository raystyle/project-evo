...
# dependencies = []
# ///
"""A/B 对照:secret-scan(A) vs code-kit scan(B)。对照不是门禁,退出码 0。
...
REPO = HERE.parents[4]
CODE_KIT_SCAN = REPO / "plugins" / "evo-adr" / "skills" / "code-kit" / "scripts" / "scan.py"
GHP = "ghp_" + ("A" * 36)
...
def _code_kit_has(finds: list[dict], file: str, rule_sub: str, history: bool | None = None) -> bool:
    for f in finds:
        if file not in str(f.get("file", "")):
            continue
        if rule_sub.lower() not in str(f.get("rule", "")).lower():
...
def main() -> int:
    ...
    b_mod = _load(CODE_KIT_SCAN, "ab_b")
    with tempfile.TemporaryDirectory(prefix="pevo-ab-") as td:
        ...
        cases = [
            ("ghp-work", _has(a_finds, "ghp.txt", "GitHub token", "worktree"), _code_kit_has(b_finds, "ghp.txt", "GitHub token", False), True, True),
            ("stripe-work", _has(a_finds, "stripe.txt", "Stripe", "worktree"), _code_kit_has(b_finds, "stripe.txt", "Stripe", False), True, False),
            ("gitlab-work", _has(a_finds, "gitlab.txt", "GitLab", "worktree"), _code_kit_has(b_finds, "gitlab.txt", "GitLab", False), True, False),
            ("hist-ghp", _has(a_finds, "old.txt", "GitHub token", "history"), _code_kit_has(b_finds, "old.txt", "GitHub token", True), True, True),
            ("hist-stripe", _has(a_finds, "old-stripe.txt", "Stripe", "history"), _code_kit_has(b_finds, "old-stripe.txt", "Stripe", True), True, False),
            ("placeholder", _has(a_finds, "ph.txt", "assigned", "worktree"), _code_kit_has(b_finds, "ph.txt", "assigned", False), False, False),
            ("aws-example", _has(a_finds, "aws.txt", "AWS", "worktree"), _code_kit_has(b_finds, "aws.txt", "AWS", False), True, True),
            ("env-file", _has(a_finds, ".env", "sensitive file", "worktree"), _code_kit_has(b_finds, ".env", "sensitive file", False), True, True),
            ("email-pii", _has(a_finds, "mail.txt", "email", "worktree"), _code_kit_has(b_finds, "mail.txt", "email", False), True, False),
        ]
        rows = []
